...
const calculateSplits = (points: TrackPoint[], totalDistance: number, smoothingWindow: number, splitDistance: number = 1.0): Split[] => {
    ...
    for (let currentDist = splitDistance; currentDist < totalDistance; currentDist += splitDistance) {
        ...
        if (splitEndPoint) {
            ...
            const powers = pointsInSplit.map(p => p.power).filter((w): w is number => !!w);
            ...
            splits.push({
                ...
                avgWatts: powers.length > 0 ? Math.round(powers.reduce((a, b) => a + b, 0) / powers.length) : null,
            });
...
/**
 * Stimates the RPE (0-10) based on Average HR relative to Max HR.
 * Using typical Zone to RPE mapping:
 * < 60% MaxHR (Z1) -> RPE 1-2
 * 60-70% MaxHR (Z2) -> RPE 3-4
 * 70-80% MaxHR (Z3) -> RPE 5-6
 * 80-90% MaxHR (Z4) -> RPE 7-8
 * > 90% MaxHR (Z5) -> RPE 9-10
 */
export const estimateTrackRPE = (stats: TrackStats, userProfile: UserProfile): number | null => {
    const maxHr = userProfile.maxHr || (userProfile.age ? 220 - userProfile.age : null);
    
    if (!stats.avgHr || !maxHr) return null;

    const percent = stats.avgHr / maxHr;

    if (percent < 0.60) return 1; // Very Easy
    if (percent < 0.65) return 2; // Easy
    if (percent < 0.70) return 3; // Moderate
    if (percent < 0.75) return 4; // Somewhat Hard
    if (percent < 0.80) return 5; // Hard
    if (percent < 0.85) return 6; 
    if (percent < 0.88) return 7; // Very Hard
    if (percent < 0.92) return 8; 
    if (percent < 0.95) return 9; // Extremely Hard
    return 10; // Max Effort
};
