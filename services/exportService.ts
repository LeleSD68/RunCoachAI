...
const generateGpxContent = (track: Track): string => {
    const trackPointsXml = track.points.map(p => {
        ...
        const hasCad = p.cad !== undefined && p.cad !== null && p.cad > 0;

        if (hasHr || hasCad) {
            extensionXml = `
            <extensions>
                <gpxtpx:TrackPointExtension>
                    ${hasHr ? `<gpxtpx:hr>${p.hr}</gpxtpx:hr>` : ''}
                    ${hasCad ? `<gpxtpx:cad>${p.cad}</gpxtpx:cad>` : ''}
                </gpxtpx:TrackPointExtension>
            </extensions>`;
        }
        ...
        <trkpt lat="${p.lat.toFixed(7)}" lon="${p.lon.toFixed(7)}">
            ...
            <time>${toISOStringWithMilliseconds(p.time)}</time>${extensionXml}
        </trkpt>`;
    }).join('');

    return `<?xml version="1.0" encoding="UTF-8"?>
<gpx version="1.1" creator="RunCoachAI"
