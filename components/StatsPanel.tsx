            ...
            <div className="space-y-2">
                ...
                {sections.summary && (
                    <div className="grid grid-cols-2 gap-2 animate-fade-in-down">
                        ...
                        <StatCard title="Tempo Muov." value={formatDuration(stats.movingDuration)} subvalue={`Tot: ${formatDuration(stats.totalDuration)}`} />
                        <StatCard title="Ritmo Medio" value={formatPace(stats.movingAvgPace)} subvalue={`Migliore: ${formatPace(Math.min(...stats.splits.map(s => s.pace)))}`} />
                        ...
                        {stats.avgHr && <StatCard title="FC Media" value={`${Math.round(stats.avgHr)} bpm`} subvalue={`Max: ${stats.maxHr} bpm`} />}
                        {stats.avgWatts && <StatCard title="Potenza (Stima)" value={`${stats.avgWatts} W`} className="col-span-2" />}
                    </div>
                )}
            </div>
