...
import { sendDirectMessage, getDirectMessages, updateTrackSharing, getTrackById, markMessagesAsRead, deleteMessage } from '../services/socialService';
...
const MiniChat: React.FC<MiniChatProps> = ({ currentUser, friend, onClose, onViewTrack, onMessagesRead }) => {
    ...
    // Delete UI
    const [msgToDelete, setMsgToDelete] = useState<DirectMessage | null>(null);
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel(`chat:${currentUser.id}:${friend.id}`)
                ...
                async (payload) => {
                    ...
                    if (newMsg.sender_id === friend.id) {
                        ...
                        setMessages(prev => {
                            ...
                            return [...prev, { id: newMsg.id, senderId: newMsg.sender_id, receiverId: newMsg.receiver_id, content: newMsg.content, createdAt: newMsg.created_at, readAt: null }];
                        });
                        await markMessagesAsRead(currentUser.id, friend.id);
                        onMessagesRead?.();
                    }
            ...
            .on(
                'postgres_changes',
                { event: 'UPDATE', schema: 'public', table: 'direct_messages', filter: `sender_id=eq.${currentUser.id}` },
                (payload) => {
    ...
    const handleDelete = async (deleteForEveryone: boolean) => {
        if (!msgToDelete || !currentUser.id) return;
        try {
            await deleteMessage(msgToDelete.id, deleteForEveryone, currentUser.id);
        ...
        if (fullAccessMode) {
            try {
                ...
                if (!currentShared.includes(friend.id)) {
                    await updateTrackSharing(track.id, track.isPublic || false, [...currentShared, friend.id], track.sharedWithGroups || []);
                }
            } catch (e) {
    ...
    return (
        <div className="fixed inset-0 z-[12000] flex items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={onClose}>
            <div className="w-full md:max-w-md bg-[#0b141a] md:border md:border-slate-700 md:rounded-2xl shadow-2xl flex flex-col h-full md:h-[600px] overflow-hidden animate-pop-in relative" onClick={(e) => e.stopPropagation()}>
                                    ...
                                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} mb-1 group relative`}>
                                        ...
                                        >
                                            ...
                                            <div className="flex justify-end items-center gap-1 mt-0.5 -mb-1 ml-2 float-right h-4">
                                                ...
                                                {isMe && (isTemp ? <ClockIcon /> : <WhatsAppTicks read={isRead} sent={true} />)}
                                            </div>
                                        </div>
                                        ...
                                        <button 
                                            ...
                                            className={`absolute top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-[#202c33] text-[#8696a0] shadow-md opacity-0 group-hover:opacity-100 transition-opacity z-10 ${isMe ? '-left-8' : '-right-8'}`}
                                        >
                                            <TrashIcon />
                                        </button>
                                    </div>
                ...
                {msgToDelete && (
                    <div className="absolute inset-0 bg-black/60 z-50 flex items-center justify-center p-6 animate-fade-in">
                        <div className="bg-[#202c33] p-4 rounded-xl shadow-2xl w-full max-w-sm border border-[#2a3942]">
                            <h3 className="text-[#e9edef] font-bold text-sm mb-4">Elimina messaggio?</h3>
                            <div className="space-y-2">
                                {msgToDelete.senderId === currentUser.id && (
                                    <button 
                                        onClick={() => handleDelete(true)}
                                        className="w-full py-3 bg-red-900/30 text-red-400 font-bold rounded-lg text-sm hover:bg-red-900/50"
                                    >
                                        Elimina per tutti
                                    </button>
                                )}
                                <button 
                                    ...
                                    className="w-full py-3 bg-[#2a3942] text-[#e9edef] font-bold rounded-lg text-sm hover:bg-[#374248]"
                                >
                                    Elimina per me
                                </button>
                                <button 
                                    onClick={() => setMsgToDelete(null)}
                                    className="w-full py-2 text-[#8696a0] text-xs font-bold uppercase mt-2"
                                >
                                    Annulla
                                </button>
                            </div>
