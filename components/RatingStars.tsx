...
const RatingStars: React.FC<RatingStarsProps> = ({ rating = 0, reason, size = 'sm', onDetailClick, onRate }) => {
    ...
    const displayReason = reason && reason.trim() !== "" 
        ? reason 
        : "Valutazione AI non ancora disponibile o calcolata.";
    ...
    const handleContainerClick = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (onDetailClick && !onRate) { // Only toggle tooltip/detail if not in rating mode
            onDetailClick(e);
        } else if (!onRate) {
            setShowTooltip(!showTooltip);
        }
    ...
    const currentRating = hoverRating !== null ? hoverRating : Math.round(rating);

    return (
        <div 
            className="relative flex items-center gap-0.5 group py-1 select-none"
            onMouseLeave={() => { setShowTooltip(false); setHoverRating(null); }}
            onClick={handleContainerClick}
            title={onRate ? "Clicca per votare" : "Clicca per il report AI dettagliato"}
        >
            {[1, 2, 3, 4, 5].map(star => (
                <svg 
                    key={star} 
                    ...
                    viewBox="0 0 20 20" 
                    fill={star <= currentRating ? "#fbbf24" : "#334155"} 
                    className={`${starSizes[size]} drop-shadow-sm transition-transform ${onRate ? 'cursor-pointer hover:scale-125' : 'pointer-events-none'}`}
            ...
            {/* Render tooltip only if NOT rating (or if we have a reason to show and user requested it) */}
            {showTooltip && !onRate && (
                <div 
                    ref={tooltipRef}
