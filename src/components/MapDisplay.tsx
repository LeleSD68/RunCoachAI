...
const StatsDisplay: React.FC<{ stats: AnimationStats, splits: Split[], currentDistance: number, visibleMetrics: Set<string> }> = ({ stats, splits, currentDistance, visibleMetrics }) => {
    ...
    return (
        <div className="absolute top-2 right-2 sm:top-4 sm:right-4 bg-slate-900/90 sm:bg-slate-800/90 backdrop-blur-md p-2 sm:p-4 rounded-xl shadow-2xl text-white z-[100] border border-slate-600/50 w-auto max-w-[calc(100%-60px)] sm:max-w-lg transition-all duration-300 animate-fade-in-down">
            <div className={`grid gap-x-3 gap-y-1 ${activeMetricsCount > 3 ? 'grid-cols-2 sm:grid-cols-4' : 'grid-flow-col auto-cols-auto'}`}>
                <div>
                    <div className="text-[8px] sm:text-xs text-slate-400 uppercase font-bold">Distanza</div>
                    <div className="text-sm sm:text-xl font-bold font-mono">{stats.distance.toFixed(2)} <span className="text-[8px] sm:text-sm text-slate-500">km</span></div>
                </div>
                {showTime && (
                    <div>
                        <div className="text-[8px] sm:text-xs text-slate-400 uppercase font-bold">Tempo</div>
                        <div className="text-sm sm:text-xl font-bold font-mono">{stats.time}</div>
                    </div>
                )}
                {showPace && (
                    <div>
                        <div className="text-[8px] sm:text-xs text-slate-400 uppercase font-bold">Ritmo</div>
                        <div className="text-sm sm:text-xl font-bold font-mono">{stats.pace}</div>
                    </div>
                )}
                {showElevation && (
                    <div>
                        <div className="text-[8px] sm:text-xs text-slate-400 uppercase font-bold">Elev.</div>
                        <div className="text-sm sm:text-xl font-bold font-mono">{stats.elevation} m</div>
                    </div>
                )}
                {showHr && (
                     <div>
                        <div className="text-[8px] sm:text-xs text-slate-400 uppercase font-bold">FC</div>
                        <div className="text-sm sm:text-xl font-bold font-mono text-red-400">{Math.round(stats.hr!)} <span className="text-[8px] sm:text-sm text-slate-500">bpm</span></div>
                    </div>
                )}
            </div>
        </div>
...
const MapDisplay: React.FC<MapDisplayProps> = ({ 
  ...
  const fitMapToBounds = useCallback(() => {
      ...
      // OPTIMIZED PADDING FOR MOBILE SPLIT VIEW
      // In the new 70/30 split, the map is small. Large paddings make the track tiny.
      const paddingOptions: any = isMobile 
        ? { padding: [10, 10] } // Minimal padding for split view
        : { padding: [40, 40] };
        ...
        if (allPoints.length > 0) bounds = L.latLngBounds(allPoints);
          ...
          if (points.length > 0) {
                ...
                if (bounds.getNorth() === bounds.getSouth() && bounds.getEast() === bounds.getWest()) {
                    ...
                    map.fitBounds(bounds, { animate: false, maxZoom: 17, padding: isMobile ? [20, 20] : [100, 100] });
                    return; 
                }
          }
